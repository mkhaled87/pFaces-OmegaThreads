...
RUN apt-get -qq install -y cmake wget git unzip build-essential libcpprest-dev opencl-c-headers  opencl-clhpp-headers ocl-icd-opencl-dev clinfo oclgrind cmake

# install pFaces (1.2.1 demo)
RUN mkdir pfaces \
	&& cd pfaces \
	&& wget https://github.com/parallall/pFaces/releases/download/Release_1.2.1d/pFaces-1.2.1-Ubuntu18.04.zip \
	&& unzip pFaces-1.2.1-Ubuntu18.04.zip
