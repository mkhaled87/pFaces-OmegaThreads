...
// no clue about DPA_NUM_LETTERS ?? check omegaDPA.cpp::TotalDPA::constructore => 2^n_io_vars


// some constants
#define MAX_POSTS @@MAX_POSTS@@             // you have to calculate this from the max volume of OARS
#define NUM_STATE_APS @@NUM_STATE_APS@@     // you get it in C++ from SymSpec::count_state_APs()
...
#define NODE_NONE ((SYMBOLIC_MAX) -2)
...
__kernel void pgame_initialize(
    ...
    // Am I (?, dummy_u, q_0) ?
    if(thread_idx_u == DUMMY_U_FLAT && thread_idx_q = 0){
    ...
    // we choose to set the BUTTOM node to (dummy,dummy,dummy)
...
__kernel void pgame_construct(
    ...
    __global ctlr_node_t* ctrl_nodes,
    ){  
